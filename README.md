...
- 🎯 Entry points to execute your program using `python -m <ai_flask_project>` or `$ ai_flask_project` with basic CLI argument parsing.
...
---
# ai_flask_project Flask Application

Awesome ai_flask_project created by kackey0-1
...
```bash
git clone https://github.com/kackey0-1/ai_flask_project ai_flask_project
cd ai_flask_project
make install
```
...
```bash
ai_flask_project create-db   # run once
ai_flask_project populate-db  # run once (optional)
ai_flask_project add-user -u admin -p 1234  # ads a user
ai_flask_project run
```
