...
original_author="kackey0-1"
original_name="ai_flask_project"
original_urlname="ai_flask_project"
original_description="Awesome ai_flask_project created by kackey0-1" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/ai_flask_project "${name}"
...
then
    ...
    echo "then run 'ai_flask_project' or 'python -m ai_flask_project'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'ai_flask_project' or 'python -m ai_flask_project' or 'poetry run ai_flask_project'"
fi
